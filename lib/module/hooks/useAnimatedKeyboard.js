...
export const useAnimatedKeyboard = () => {
  ...
  useEffect(() => {
    ...
    const handleOnKeyboardShow = event => {
      scheduleOnUI(handleKeyboardEvent, KEYBOARD_STATUS.SHOWN, event.endCoordinates.height, event.duration, event.easing, SCREEN_HEIGHT - event.endCoordinates.height - event.endCoordinates.screenY);
    };
    const handleOnKeyboardHide = event => {
      scheduleOnUI(handleKeyboardEvent, KEYBOARD_STATUS.HIDDEN, event.endCoordinates.height, event.duration, event.easing);
    };
