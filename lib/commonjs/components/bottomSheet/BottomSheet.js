...
const BottomSheetComponent = /*#__PURE__*/(0, _react.forwardRef)(function BottomSheet(props, ref) {
  ...
  const animateToPositionCompleted = (0, _react.useCallback)(function animateToPositionCompleted(isFinished) {
    ...
    if (__DEV__) {
      (0, _reactNativeWorklets.scheduleOnRN)(_utilities.print, {
        component: 'BottomSheet',
    ...
    if (nextIndex !== animatedCurrentIndex.get()) {
      (0, _reactNativeWorklets.scheduleOnRN)(handleOnChange, nextIndex, nextPosition);
    }
    if (nextIndex === -1) {
      (0, _reactNativeWorklets.scheduleOnRN)(handleOnClose);
    }
  ...
  const animateToPosition = (0, _react.useCallback)(function animateToPosition(position, source, velocity = 0, configs) {
     ...
     */
    (0, _reactNativeWorklets.scheduleOnRN)(handleOnAnimate, index, position);
    ...
    const clampedVelocity = Math.max(-MAX_VELOCITY, Math.min(MAX_VELOCITY, velocity));
    const currentPosition = animatedPosition.value;
    const movingTowardsTarget = position > currentPosition && clampedVelocity > 0 || position < currentPosition && clampedVelocity < 0;
  ...
  const handleSnapToIndex = (0, _hooks.useStableCallback)(function handleSnapToIndex(index, animationConfigs) {
     ...
     */
    isInTemporaryPosition.value = false;
    (0, _reactNativeWorklets.scheduleOnUI)(animateToPosition, targetPosition, _constants.ANIMATION_SOURCE.USER, 0, animationConfigs);
  });
