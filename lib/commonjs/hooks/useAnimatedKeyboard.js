...
const useAnimatedKeyboard = () => {
  ...
  (0, _react.useEffect)(() => {
    ...
    const handleOnKeyboardShow = event => {
      (0, _reactNativeWorklets.scheduleOnUI)(handleKeyboardEvent, _constants.KEYBOARD_STATUS.SHOWN, event.endCoordinates.height, event.duration, event.easing, SCREEN_HEIGHT - event.endCoordinates.height - event.endCoordinates.screenY);
    };
    const handleOnKeyboardHide = event => {
      (0, _reactNativeWorklets.scheduleOnUI)(handleKeyboardEvent, _constants.KEYBOARD_STATUS.HIDDEN, event.endCoordinates.height, event.duration, event.easing);
    };
